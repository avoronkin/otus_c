...
int main(int argc, char *argv[]) {
  ...
  while ((ch = fgetc(file_ptr)) != EOF) {
    ...
    if (fseek(file_ptr, header_offset, SEEK_SET) != 0) {
      fprintf(stderr,
              "Чтение signature: Не удалось переместить указатель в файле");
      exit(EXIT_FAILURE);
    }
    ...
    if (fseek(file_ptr, header_offset + 26, SEEK_SET) != 0) {
      fprintf(
          stderr,
          "Чтение file_name_length: Не удалось переместить указатель в файле");
      exit(EXIT_FAILURE);
    }
    ...
    if (fseek(file_ptr, header_offset + 30, SEEK_SET) != 0) {
      fprintf(stderr,
              "Чтение file_name: Не удалось переместить указатель в файле\n");
      exit(EXIT_FAILURE);
    }
