...
{
  *forecasts = malloc(sizeof(MW_forecasts));
  if (forecasts == NULL) {
...
mw_forecasts_fetch(MW_forecasts* forecasts, int woeid)
{
  ...
  int url_length = snprintf(NULL, 0, "%s/%d/%d/%d/%d/", endpoint_url, woeid, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
  ...
  CURLcode curl_code = fetch(url, &http_body);
  free(url);
  ...
  MW_code mw_code = mw_forecasts_fill_from_response(forecasts, http_body.data);
  if (mw_code) {
    free(http_body.data);
    return mw_code;
  }
...
mw_locations_search(MW_locations* locations, char* location_name)
{
  ...
  MW_code mw_code = mw_locations_fill_from_response(locations, http_body.data);
  if (mw_code != MW_OK) {
    free(http_body.data);
    return mw_code;
  }
